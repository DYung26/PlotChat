  ...
  return (
    <aside className={clsx(
      ...
      <nav className="p-1 space-y-1 shrink-0">
        <Button
          variant="ghost"
          onClick={() => setNewProjectsOpen(true)}
        ...
        <Button variant="ghost" className="w-full justify-start cursor-pointer p-1 border-t border-b !bg-gray-100">
          ...
          {expanded && "Search Projects"}
        </Button>
      </nav>

      <div className="flex-1 mt-1 border-t border-b shrink-0">
        {expanded && <p className="px-3 py-2 text-xs text-muted-foreground font-semibold uppercase">Projects</p>}
        <ScrollArea className="flex-1 px-2 overflow-y-auto mt-2 h-[calc(100vh-300px)]">
        {/*h-[calc(100vh-220px)]*/}
          <ul className="space-y-2 pb-1">
            {[...projects].reverse().map(a => (
              <li key={a.id}> 
                <button
                  onClick={() => {
                    startTransition(() => {
      ...
      <NewProjectDialog open={newProjectsOpen} onOpenChange={setNewProjectsOpen} />
